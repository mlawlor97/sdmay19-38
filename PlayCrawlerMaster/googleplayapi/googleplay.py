...
import PlayCrawlerMaster.googleplayapi.googleplay_pb2 as PB2
...
class GooglePlayPassword:
    ...
    """
    KEY = "AAAAgMom/1a/v0lblO2Ubrt60J2gcuXSljGFQXgcyZWveWLEwo6prwgi3iJIZdodyhKZQrNWp5nKJ3srRXcUW" \
...
class GooglePlayAPI(object):
    ...
    def toDict(self, protoObj):
        ...
        for po in protoObj:
            ...
            for fielddesc, value in po.ListFields():
                if fielddesc.type == descriptor.FieldDescriptor.TYPE_GROUP \
                        or isinstance(value, RepeatedCompositeFieldContainer) or isinstance(value, Message):
        ...
        if (authSubToken is not None):
            ...
            if (email is None or password is None):
                ...
                if response.status_code != http.client.OK:
                    logging.error(
                        '{0} Play Store login failed, statuscode {1}: {2}'.format(self.androidId, response.status_code,
                                                                                  response.content))
                else:
    ...
    def executeRequestApi2(self, path, sdk=23, agentvername=None, agentvercode=None, devicename="sailfish",
        ...
        user_agent = "Android-Finsky/" + agentvername + " (api=3,versionCode=" + agentvercode + ",sdk=" + str(
            sdk) + ",device=" + devicename + ",hardware=" + devicename + ",product=" + devicename + ",build=NZZ99Z:user)"
        ...
        if (datapost is None and path in self.preFetch):
            ...
            headers = {
                ...
                "X-DFE-Cookie": base64.standard_b64encode(
                    b'\x08\xa9\x0f\x10\x01\x18\x00"\x02' + self.regionCookie.encode(
                        'utf-8') + '(\x012<CisaKQoTMzcxNzUwNjQ2MzkzMzkzMjA1NBISChAxNTEwMTM0ODU0NTE4MDU2'.encode(
        ...
        message = PB2.ResponseWrapper.FromString(data)
    ...
    def bulkDetails(self, packageNames, sdk):
        ...
        path = "bulkDetails"
        req = PB2.BulkDetailsRequest()
        req.docid.extend(packageNames)
        data = req.SerializeToString()
        (status_code, message) = self.executeRequestApi2(path, sdk=sdk, datapost=data,
    ...
    def download(self, packageName, versionCode, offerType=1, agentvername=None, agentvercode=None,
        ...
        app."""
        (status_code, message) = self.executeRequestApi2(
            path="delivery?ot=%d&doc=%s&vc=%d&shh=%s" % (offerType, packageName, versionCode, "1"),
            agentvername=agentvername, agentvercode=agentvercode, devicename=devicename)

        if status_code == http.client.OK:
            # app is not in user library
            if message.payload.deliveryResponse.status == 3:
                (status_code, message) = self.executeRequestApi2(path="purchase", datapost="ot=%d&doc=%s&vc=%d" % (
                offerType, packageName, versionCode))
                if status_code != http.client.OK:
                    return RequestResult(status_code, None)
            ...
            if response.status_code != http.client.OK:
                ...
                return RequestResult(response.status_code,
                                     response.content)  # take care that this response is different from the other return functions, it concerns the APK content itself (of the 2nd request)
